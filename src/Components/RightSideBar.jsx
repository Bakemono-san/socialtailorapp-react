...
export default function Sidebar(props) {
  ...
  return (
    <div
      className={
        props.color +
        `  bg-white hidden md:flex md:py-4 md:px-5 md:flex-col md:gap-10 md:h-full text-white md:rounded w-full min-h-12 shadow md:w-24 tv:w-max tv:bg-blue-400 2xl:w-full`
      }
    >
      ...
      <div className="flex-1 md:flex-col md:gap-8 justify-between md:justify-normal flex">
        {value.message.map((message, index) => {
          return (
            <div className="tv:flex justify-between gap-4  items-center tv:gap-8">
              <SidebarItem
                key={message.Users_UsersDiscussions_receiverIdToUsers.id}
                image={
                  message.Users_UsersDiscussions_receiverIdToUsers.photoProfile
                }
